...
#   ./CreateNetCDFs.sh /media/denis/dados/_COM_BACKUP/NN_BAM1D/bam1d_data/model_dataout/IOP2014_CRDRRTMG_HUMO__TIED_GRE_90_savedeach2_64 2014 64
#
# -------------------------------- WARNING ------------------------------
# Enver Ramirez
# April 12, 2019
...
PREC_GOA="prec_goamazon_${year}"
find ${fdir} -name "${PREC_GOA}*.ctl" -print0 | xargs -0 >> lista

tdef=`more ${fdir}/${PREC_GOA}.ctl |grep tdef |awk '{print $4}'`
...
echo $idx
ncap2 -O -s '@units="20 minutes since '${year}'-1-1 00:00:00";time=udunits(time,@units);time@units=@units;time={'$idx'}' tmp2.nc ${newfile}nc
rm tmp.nc tmp2.nc
idx=`expr $idx + 1`

#if [ $idx -gt 9 ] ; then break; fi
done
# cdo cat ${PREC_GOA}*.nc ${PREC_GOA}___ALL.nc
rm newzaxis lista
